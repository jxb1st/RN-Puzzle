...
class ViTBlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_dim, dropout_rate=0.1):
        ...
        self.self_attention = nn.MultiheadAttention(dim, num_heads, dropout=dropout_rate)
...
class saViTBlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_dim, threshold, dropout_rate=0.1):
        super(saViTBlock, self).__init__()
        self.vit_block = ViTBlock(dim, num_heads, mlp_dim, dropout_rate)
        self.rn_l = RN_L(feature_channels=dim, threshold=threshold)
...
class G_Net(nn.Module):
    ...
    def __init__(self, input_channels, residual_blocks, threshold):
        ...
        for _ in range(residual_blocks):
            block = saViTBlock(dim=256, num_heads=num_heads, mlp_dim=mlp_dim, threshold=threshold)
            blocks.append(block)
