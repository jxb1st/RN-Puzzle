...
if __name__ == '__main__':
    ...
    print(f"Training data loader length: {len(training_data_loader)}")
    # 打印第一个批次的数据形状
    if len(training_data_loader) > 0:
        first_batch = next(iter(training_data_loader))
        print(f"First batch shapes - gt: {first_batch[0].shape}, mask: {first_batch[1].shape}")
    else:
        print("Training data loader is empty")
